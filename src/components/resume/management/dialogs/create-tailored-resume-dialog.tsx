...
export function CreateTailoredResumeDialog({ children, baseResumes, profile }: CreateTailoredResumeDialogProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // 4.5 Generate Cover Letter
      // We do this concurrently or after tailoring, but before final creation
      let coverLetterContent = '';
      try {
        // Import dynamically to avoid circular dependencies if any, 
        // though here we imported from ai.ts which is fine.
        const { generateCoverLetterContent } = await import("@/utils/actions/resumes/ai");

        coverLetterContent = await generateCoverLetterContent(
          baseResume,
          jobDescription,
          {
            model: selectedModel || '',
        ...
        console.warn("Autogenerated cover letter failed, continuing without it:", clError);
        // We continue without cover letter if it fails
      }
      ...
      // 5. Create the tailored resume with job reference AND cover letter
      const resume = await createTailoredResume(
        baseResume,
        jobEntry.id,
        ...
        tailoredContent,
        coverLetterContent // Pass the generated cover letter
      );
